...
HOOK_MARKER = "# Generated by logwright"
...
def install_commit_msg_hook(
    *,
    ...
    configured_hooks_path: Path | None = None
    if not _is_repo_local_hook_path(hook_path, repo=repo, repo_git_dir=repo_git_dir):
        configured_hooks_path = repo_git_dir / "hooks"
        configured_hooks_path.mkdir(parents=True, exist_ok=True)
        run_git(repo, "config", "--local", "core.hooksPath", str(configured_hooks_path))
        hook_path = git_path(repo, "hooks/commit-msg")
    ...
    script = "\n".join(
        [
            "#!/bin/sh",
            HOOK_MARKER,
            "# Re-run `logwright --install-commit-msg-hook --force` to update this hook.",
            command,
            "",
        ]
    )
    ...
    updated_existing = hook_path.exists()
    if updated_existing:
        current = hook_path.read_text(encoding="utf-8")
        managed = HOOK_MARKER in current
        if not managed and not force:
            raise GitError(
                f"refusing to overwrite existing commit-msg hook at {hook_path}; rerun with --force"
            )
        if not managed:
            backup_path = _unique_backup_path(hook_path)
    ...
    return HookInstallResult(
        ...
        configured_hooks_path=(
            str(configured_hooks_path) if configured_hooks_path is not None else None
        ),
...
def render_hook_install_result(result: HookInstallResult) -> str:
    lines = [
        f"Installed commit-msg hook in {result.repo_path}",
        f"Hook path: {result.hook_path}",
        (
            f"Provider: {result.provider} ({result.model})"
            if result.model
            else f"Provider: {result.provider}"
        ),
        f"Minimum score: {result.min_score}/10",
    ]
    if result.configured_hooks_path:
        lines.append(f"Configured local core.hooksPath: {result.configured_hooks_path}")
    lines.extend(
    ...
    elif result.updated_existing:
        lines.append("Updated existing logwright-managed hook.")
    else:
        lines.append("Created new hook.")
    return "\n".join(lines)
...
def _render_usage_lines(usage: UsageStats) -> list[str]:
    ...
    if amount is None:
        ...
        lines.append(f"Estimated API cost: ${amount:.4f} ({cost['cost_note']})")
    return lines
...
def _hook_command(
    *,
    ...
    exec_command = " ".join('"$1"' if arg == '"$1"' else shlex.quote(arg) for arg in args)
    source_root = _hook_source_root()
    if source_root is None:
        return f"exec {exec_command}"
    pythonpath_prefix = (
...
def _unique_backup_path(hook_path: Path) -> Path:
    base = hook_path.with_name("commit-msg.before-logwright")
    if not base.exists():
        return base
    index = 1
    while True:
        candidate = hook_path.with_name(f"commit-msg.before-logwright.{index}")
        if not candidate.exists():
            return candidate
        index += 1


def _hook_source_root() -> Path | None:
    source_root = Path(__file__).resolve().parents[1]
    if (source_root / "pyproject.toml").exists() and (source_root / "logwright").is_dir():
        return source_root
    return None


def _is_repo_local_hook_path(hook_path: Path, *, repo: Path, repo_git_dir: Path) -> bool:
    resolved = hook_path.resolve()
    return _is_relative_to(resolved, repo.resolve()) or _is_relative_to(
        resolved, repo_git_dir.resolve()
    )
